...
impl State {
    ...
    fn overdraw_terminal_physical(&mut self, buf: &mut [u32], pw: usize, ph: usize) {
        ...
        // Cell grid, short of the scrollbar gutter. The upscaled logical layer
        // already carries the full terminal viewport backdrop.
        let clip_right =
    ...
    fn compose_physical(&mut self, buf: &mut [u32]) {
        ...
        if self.upscale_cols_key != cols_key {
            ...
            self.upscale_cols
                .extend((0..pw).map(|px| ((px as f64 / s) as usize).min(lw - 1) as u32));
    ...
    fn shown(&self) -> Option<NodeId> {
        ...
        self.tree
            .first_leaf_matching(self.sidebar.primary(), &|leaf| {
                self.sessions.contains_key(&leaf)
            })
    }
...
";
    ...
    fn embedded_fallback_renders_prompt_arrow_immediately() {
        ...
        assert!(
            g.w > 0 && g.h > 0,
            "prompt arrow must not wait for async OS fallbacks"
        );
    }
